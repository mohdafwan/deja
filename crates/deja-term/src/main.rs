    ...
    fn tab_bar(&mut self, ui: &mut egui::Ui, ctx: &egui::Context, theme: Theme) {
        ...
        ui.horizontal(|ui| {
            ...
            for (i, t) in self.tabs.iter().enumerate() {
                ...
                let txt_col = if selected { theme.fg } else { theme.muted };

                // chip = tab title + close, ek rounded frame me
                let resp = egui::Frame::default()
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.horizontal(|ui| {
                            if selected {
                                ui.label(egui::RichText::new("●").color(theme.accent).size(8.0));
                            }
                            ui.label(egui::RichText::new(t.title()).color(txt_col).size(13.0));
                            if ui
                                .add(
                                    egui::Button::new(
                                        egui::RichText::new("×").color(theme.muted).size(13.0),
                                    )
                                    .frame(false),
                                )
                                .clicked()
            ...
            // new tab
            if ui
                .add(egui::Button::new(egui::RichText::new("+").color(theme.muted).size(16.0)).frame(false))
    ...
    fn render_input(&mut self, ui: &mut egui::Ui, font: &egui::FontId, theme: Theme) {
        ...
        ui.horizontal(|ui| {
            ...
            egui::Frame::default()
                ...
                .show(ui, |ui| {
                    ui.label(egui::RichText::new(format!("▢ {chip}")).color(theme.path).size(12.0));
                });
...
/// Header: `~/path git:(branch)` — path + branch color-coded.
...
fn render_card(
    ...
    let inner = deja_frame(ui).show(ui, |ui| {
        ...
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                if ui.small_button("⧉").on_hover_text("copy output").clicked() {
                    ui.ctx().copy_text(block_text(output));
                }
                if b.when > 0 {
                    ui.label(
                        egui::RichText::new(deja_core::diff::humanize_since(b.when))
                            .color(theme.muted)
                            .size(11.0),
                    );
                }
